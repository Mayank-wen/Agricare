...
const RecommendationSection = styled.div`
  ...
  min-width: 300px; // Added min-width to maintain size
`;
...
const Sidebar = styled.aside`
  width: 700px; // Increased from 350px to fit both columns
  min-width: 700px;
...
const SatelliteOverlay = memo(({ visible, opacity, bounds, map }) => {
  useEffect(() => {
    if (!visible || !bounds || !map) return;
    ...
    const loadHeatmap = async () => {
      try {
        ...
        // Convert NDVI points to OpenLayers features
        const features = ndviData.points.map((point) => {
          return new Feature({
        ...
        heatmapLayer = new HeatmapLayer({
          ...
          radius: 10,
          opacity: opacity || 0.7,
          gradient: ["#d73027", "#fdae61", "#a6d96a", "#1a9850"],
        });
...
const MapComponent = ({ onLocationSelect, center }) => {
  ...
  const [isBasemap, setIsBasemap] = useState(true); // To track current map type
  ...
  useEffect(() => {
    ...
    const toggleMapType = () => {
      ...
      osmLayer.setVisible(isBasemap);
      satelliteLayer.setVisible(!isBasemap);
    };
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%" }}>
      ...
      <MapControls isBasemap={isBasemap} setIsBasemap={setIsBasemap} />
    </div>
  );
};
        ...
        <SidebarContent>
          <FormSection>
            <div>
              ...
              {location && (
                <LocationInfo>
                  Selected: {location.lat.toFixed(4)}°N,{" "}
          ...
          {recommendation && (
            <RecommendationPanel>
              <RecommendationSection>
                ...
                />
                <p>
                  Based on your soil and weather conditions, we recommend
                  growing:
                </p>
                <CropTitle>{recommendation.recommended_crop}</CropTitle>
                {recommendation.success && (
                  <div style={{ color: "#27ae60", marginTop: "10px" }}>
                    ✅ This recommendation is based on analysis of your
...
// Add MapControls component
const MapControls = ({ isBasemap, setIsBasemap }) => {
  return (
    <ControlsContainer>
      <ControlGroup>
        <MapToggle
          onClick={() => setIsBasemap(true)}
          className={isBasemap ? "active" : ""}
        >
          Street Map
        </MapToggle>
        <MapToggle
          onClick={() => setIsBasemap(false)}
          className={!isBasemap ? "active" : ""}
        >
          Satellite
        </MapToggle>
